...
ilosc_plikow_testowych = len(nicki_badanych)-1
...
przebiegi = [[0],[1],[2],[3], [4],[5],[6] , [7,8]] # ostatni element = przbieg testowy
...
def wczytaj_dane(nr_przebiegu=0, czy_testowy=False):
    global X, y_txt, X_test, y_test, y, y_train, y_validate, X_validate,  X_train
    
    if dane_wieloplikowe:
      zarostek_plikow = "_"+etap_pliku+nazwa_pliku+oznaczenie+".npy"
...
for i in range(1, ilosc_plikow_testowych):
    wczytaj_dane(i)
...
for i in range(0, ilosc_plikow_testowych):
    wczytaj_dane(i)
    szkol_model(dane_wieloplikowe=True,pierwsze_szkolenie=False)
